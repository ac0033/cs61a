...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        pruned_branches = [prune_leaves(branch, vals) for branch in branches(t)]
        live_branches = [b for b in pruned_branches if b]
        return tree(label(t), live_branches)
...
def balanced(t):
    ...
    for branch in branches(t):
        if not balanced(branch):
            return False
        branch_sums.append(sum_tree(branch))
        
    return len(set(branch_sums)) <= 1
