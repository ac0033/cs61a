...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lower=None, upper=None) -> Bee | None:
        ...
        distance = 0
        while not bees_place.is_hive:
            if bees_place.bees and distance >= lower and distance <= upper:
                return random_bee(bees_place.bees) # REPLACE THIS LINE
            else:
                distance += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, damage_taken: float):
        ...
        total_damage = damage_taken + self.damage if damage_taken >= self.health else damage_taken
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other: Ant) -> bool:
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return (not self.ant_contained or self.ant_contained is other) and not other.is_container
        # END Problem 8a
    ...
    def action(self, gamestate: GameState):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained: self.ant_contained.action(gamestate)
        # END Problem 8a
...
def _get_subclasses(base_class):
    """Return a flat list of all subclasses of base_class."""
    all_types = []
...
def ant_types() -> list:
    ...
    return [t for t in _get_subclasses(Ant) if t.implemented]


def bee_types() -> list:
    """Return a list of all implemented Bee classes."""
    return _get_subclasses(Bee)
