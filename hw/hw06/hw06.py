...
class VendingMachine:
    ...
    def vend(self) -> str:
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.stock > 0 and self.balance >= self.sell_price:
            change = self.balance - self.sell_price
            self.stock -= 1
            self.balance = 0
            if change > 0:   
                message = f'Here is your {self.product_name} and ${change} change.'
            else:
                message = f'Here is your {self.product_name}.'
        elif self.stock == 0:
            message = 'Nothing left to vend. Please restock.'
        else:
            message = f'Please add ${self.sell_price - self.balance} more funds.'
        return message     
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        largest = max(t.branches, key=lambda x: abs(x.label))
